...
pwmPin = 18 #Pin with PWM capability please reference https://pinout.xyz/pinout/pin33_gpio13
tachoPin = 17 #Pin where the tacho is plugged in
desiredTemp = 45
maxTemp = 70 #At this temperature the fan will run at 100%
...
rpmFilePath = "/etc/pwmfan/rpm" #Path to where a file with the rpm number should be saved. So other apps can access it.
tempFilePath = "/etc/pwmfan/temp"
...
class FreqThread(threading.Thread):
	def __init__(self, queue, tempPin, rpmReadCycle, filepath, args=(), kwargs=None):
		...
		pi.set_mode(self.tpin, pigpio.INPUT)
		self.fpath = filepath
	...
	def getRPM(self):
		...
		for c in range(self.cycle):
			...
			pinread = pi.wait_for_edge(self.tpin, pigpio.FALLING_EDGE, 1.0)
			if pinread is None:
				break
...
def setup():
	...
	#pwmctr.start(100)
	pi.set_pull_up_down(tachoPin, pigpio.PUD_UP)
	pi.hardware_PWM(pwmPin, pwmFre, 1000000)
...
def changeDuty(): #calculates the needed PWM duty and changes it
	global sum, pwmDuty, dutyAverage, mWaited
	temp = getCPUtemp()
	...
	pi.hardware_PWM(pwmPin, pwmFre, 10000*pwmDuty)
	print("actualTemp {:4.2f} TempDiff {:4.2f} pDiff {:4.2f} pwmDuty {:5.0f}".format(temp, diff, pDiff, pwmDuty))
	f1 = open(tempFilePath, 'w')
	f1.write("actualTemp {:4.2f} TempDiff {:4.2f} pDiff {:4.2f} pwmDuty {:5.0f}".format(temp, diff, pDiff, pwmDuty))
	f1.close()
	return()
def fanOFF():
	
	#pwmctr.ChangeDutyCycle(0)
	pi.hardware_PWM(pwmPin, pwmFre, 0)
	return
